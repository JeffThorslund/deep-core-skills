...
Issues for this repo live in **Linear** (team: `Engineering`). Use the Linear MCP tools for all operations:
...
The five state labels and two category labels below **already exist verbatim** in the `Engineering` team — apply them by their literal names; do not create new ones.
...
5. **Apply the outcome:**
   ...
   - `wontfix` (bug) — apply the `wontfix` label, post a polite explanation, then move to the `Canceled` state.
   - `wontfix` (enhancement) — write to `.out-of-scope/`, link to it from a comment, apply the `wontfix` label, then move to the `Canceled` state ([OUT-OF-SCOPE.md](OUT-OF-SCOPE.md)).
